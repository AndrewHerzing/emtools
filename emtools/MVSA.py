...
def decompositionRGB(data, comps=None, energyrange=None, intensityrange=None,
                     labels=None):
    if comps is None:
        comps = [0, 1, 2]
    if energyrange is None:
        energyrange = [data.axes_manager[-1].axis[0],
                       data.axes_manager[-1].axis[-1]]
    if intensityrange is None:
        intensityrange = [data.learning_results.factors.min(),
                          data.learning_results.factors.max()]
    fig, ax = plt.subplots(2, 1)
    colors = ['red', 'green', 'blue']

    if labels is None:
        algorithm = data.learning_results.decomposition_algorithm.upper()
        labels = [algorithm + ' #%s' % (comps[0]+1), algorithm +
                  ' #%s' % (comps[1]+1), algorithm + ' #%s' % (comps[2]+1)]
    ...
    rgb = plot_RGB([images[comps[0]],
                    images[comps[1]],
                    images[comps[2]]],
                   dont_plot=True)

    plot_spectra([spectra[comps[0]],
                  spectra[comps[1]],
                  spectra[comps[2]]],
                 fig=fig,
                 ax=ax[0],
    ...
    while (not_converged) and (iter < max_iter):
        for j in range(0, k-1):
            for l in range(j+1, k):
                """Calculate optimal 2-D rotation angle for columns j,l"""
                phi_max = np.angle(
                                   n*np.sum(
                                    (matrix[:, j]+1j*matrix[:, l])**4) -
                                   np.sum(
                                    ((matrix[:, j]+1j*matrix[:, l])**2)**2))/4
                sub_rot = [[np.cos(phi_max),
                            -np.sin(phi_max)],
                           [np.sin(phi_max),
                            np.cos(phi_max)]]

                matrix[:, np.ix_([j, l])] = \
                    matrix[:, np.ix_([j, l])].dot(sub_rot)
...
def PCA(si, normalize=False, newsize=None):
    if si._lazy:
        ...
        ax.set_yscale('log')
        ax.scatter(np.arange(0, 30),
                   data.learning_results.explained_variance_ratio[0:30],
                   s=80)
...
def showResults(model, ncomps=3, shape=None, slice=0):
    ...
    if len(model.data.shape) == 2:
        ...
        scores = model.learning_results.\
            loadings[:, 0:ncomps].\
            reshape(np.append([nrows, ncols, ntilts], ncomps))
        ...
        scores = model.\
            learning_results.\
            loadings[:, 0:ncomps].\
            reshape(np.append([nrows, ncols], ncomps))
        for i in range(0, ncomps):
            fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 4))
            ax2.plot(loadings[:, i])
            ax2.set_title('Spectral Component #%s' % str(i))
...
def SpatialSimplicity(model, ncomps=None, shape=None, slice=0):
    if len(model.data.shape) == 2:
        ...
        for i in range(0, ncomps):
            ...
            ax1.imshow(rotate_im[:, :, slice, i],
                       cmap='afmhot',
                       interpolation='none',
                       clim=[0.0, rotate_im[:, :, slice, i].max()])
        ...
        scores = model.\
            learning_results.\
            loadings[:, 0:ncomps].\
            reshape(np.append(np.array(model.data.shape[0:2]), ncomps))
        rot_spec, rotate_spec = varimax(loadings)
        rotate_im = scores.dot(rot_spec)
        flip = np.sign(rotate_spec.sum(0))
        rotate_spec = rotate_spec*flip
        ...
        for i in range(0, ncomps):
            ...
            ax2.plot(rotate_spec[:, i])
            ax2.set_title('Rotated Spectral Component #%s' % str(i))
            ax1.imshow(rotate_im[:, :, i],
                       cmap='afmhot',
                       interpolation='none',
                       clim=[0.0, rotate_im[:, :, i].max()])
            ax1.set_title('Rotated Spatial Component #%s' % str(i))
        plt.show()
        ...
        scores = model.\
            learning_results.\
            loadings[:, 0:ncomps].\
            reshape(np.append([ncols, nrows, ntilts], ncomps))
        rot_spec, rotate_spec = varimax(loadings)
        rotate_im = scores.dot(rot_spec)
        flip = np.sign(rotate_spec.sum(0))
        rotate_spec = rotate_spec*flip
        ...
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 4))
        rgb = Color.rgboverlay(np.reshape(rotate_im[:, :, slice, :],
                                          [ncols, nrows, ncomps]))
        ax1.imshow(rgb, interpolation='none', clim=[200, rgb.max()])
        ax1.set_title('Color Overlay')
        colors = ['red', 'green', 'blue',
