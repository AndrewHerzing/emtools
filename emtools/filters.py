...
def butter_bpf(data,Dl,Dh,n=1):
    """
    Function to apply a Butterworth bandpass filter to an image or stack of images
    ...
    [u,v] = np.meshgrid(np.arange(-np.floor(width/2),np.floor(width/2)),np.arange(-np.floor(height/2),np.floor(height/2)))
...
def bandFilter(data,in_radius=9,out_radius=60):
    ...
    mask = np.logical_xor(x**2 + y**2 <= out_radius**2,x**2 + y**2 < in_radius**2)

    imFreq = np.fft.fft2(im)
    imAmp = np.fft.fftshift(np.abs(imFreq))
    imFilt = np.fft.fftshift(imFreq) * mask
    imNew = np.real(np.fft.ifft2(np.fft.ifftshift(imFilt)))
    
    #out.data = np.rot90(np.float32(imNew),axes=(2,1))
