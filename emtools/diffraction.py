    ...
    if unit_cell == 'cubic':
        ...
        if not c_dim:
            raise ValueError("Must provide lattice constant 'c' for unit cell\
                type '%s'." % unit_cell)
        d_hkl = np.sqrt(1 / (((h_index**2 + k_index**2) / a_dim**2) + (l_index**2 / c_dim**2)))
        ...
        if not c_dim:
            raise ValueError("Must provide lattice constant 'c' for unit cell\
                type '%s'." % unit_cell)
        d_hkl = np.sqrt(1 / (4 / 3 * ((h_index**2 + h_index * k_index + k_index**2) / a_dim**2) + (l_index**2 / c_dim**2)))
        ...
        d_hkl = np.sqrt((1 / (((h_index**2 + h_index * k_index + k_index**2) * (np.sin(alpha))**2 + 2 * (h_index * k_index + k_index * l_index + l_index * h_index) * ((np.cos(alpha))**2 - np.cos(alpha))) / (a_dim**2 * (1 + 2 * (np.cos(alpha))**3 - 3 * (np.cos(alpha))**2)))))
        ...
        if not c_dim:
            raise ValueError("Must provide lattice constant 'c' for unit cell\
                type '%s'." % unit_cell)
        d_hkl = np.sqrt(1 / ((h_index**2 / a_dim**2) + (k_index**2 / b_dim**2) + (l_index**2 / c_dim**2)))
        ...
        if not beta:
            raise ValueError("Must provide lattice constant 'beta' for unit\
                cell type '%s'." % unit_cell)
        d_hkl = np.sqrt(1 / ((1 / (np.sin(beta))**2) * ((h_index**2 / a_dim**2) + ((k_index**2 * (np.sin(beta))**2) / b_dim**2) + (l_index**2 / c_dim**2) - (2 * h_index * l_index * np.cos(beta) / (a_dim * c_dim)))))
        ...
        d_hkl = np.sqrt(1 / ((1 / vol**2) * (s11 * h_index**2 + s22 * k_index**2 + s33 * l_index**2 + 2 * s12 * h_index * k_index + 2 * s23 * k_index * l_index + 2 * s13 * h_index * l_index)))
    else:
