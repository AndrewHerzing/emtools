...
def plotEDS(spec,axis=None,peaklabels=None,linecolor='red',energyrange=None,intensityrange=None,horz_offset=None,vert_offset=None,fontsize=8):
    if axis is None:
    ...
    if peaklabels:
        if peaklabels is True:
            ...
            raise ValueError('Unknown format for "peaklabels". Must be boolean or list')
            return
        ...
        for i in range(0,len(peaklabels)):
            ...
            energy = elements.elements[element]['Atomic_properties']['Xray_lines'][line]['energy (keV)']
            ...
            if (x_pos<horz_min) or (x_pos>horz_max):
                ...
                axis.text(x=x_pos,y=y_pos,s=peaklabels[i],rotation=90,rotation_mode='anchor',size=fontsize)
    if out:
        return(figure,axis)
    else:
        return
